...
void Anchor_SendClientData() {
    ...
    payload["data"]["color"] = "1"; //CVarGetColor24("gRemote.AnchorColor", { 100, 255, 100 });
...
void GameInteractorAnchor::HandleRemoteJson(nlohmann::json payload) {
    ...
    if (payload["type"] == "GIVE_ITEM") {
        u8 itemID = payload["getItemId"].get<int16_t>();
        gSaveContext.save.saveInfo.inventory.items[itemID] = itemID;
        AnchorClient anchorClient = GameInteractorAnchor::AnchorClients[payload["clientId"].get<uint32_t>()];
        // Anchor_DisplayMessage({
        //                 .itemIcon = "test", //SohUtils::GetIconNameFromItemID(getItemEntry.itemId),
        //                 .prefix = itemID,
        //                 .message = "from",
